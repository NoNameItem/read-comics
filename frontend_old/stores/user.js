...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    const image = computed(
      () => images.value?.image ?? defaultImages[gender.value?.value ?? "O"],
    );
    const thumbnail = computed(
      () =>
        images.value?.thumbnail ??
        defaultThumbnails[gender.value?.value ?? "O"],
    );
    const loggedIn = computed(() => !!accessToken.value);
    const isSuperuserOrStaff = computed(
      () => isSuperuser.value || isStaff.value,
    );
