...
def get_issues_queryset(volume, user=None):
    q = (
        volume.issues.filter(comicvine_status="MATCHED")
        .order_by("numerical_number", "number")
        .annotate(
            parent_slug=F("volume__slug"),
            badge_name=Concat(Value("#"), F("number"), Value(" "), F("name"), output_field=TextField()),
            desc=F("cover_date"),
        )
    )
...
def get_characters_queryset(volume):
    return (
        Character.objects.filter(comicvine_status="MATCHED")
        .filter(issues__volume=volume)
        .annotate(
...
def get_died_queryset(volume):
    return (
        Character.objects.filter(comicvine_status="MATCHED")
        .filter(died_in_issues__volume=volume)
        .distinct()
        .order_by("name", "id")
    )
...
def get_concepts_queryset(volume):
    return (
        Concept.objects.filter(comicvine_status="MATCHED")
        .filter(issues__volume=volume)
        .annotate(
            issues_count=Count("issues", filter=Q(issues__volume=volume)),
            desc=Concat(Value("Appeared in "), F("issues_count"), Value(" issue(s)"), output_field=TextField()),
...
def get_locations_queryset(volume):
    return (
        Location.objects.filter(comicvine_status="MATCHED")
        .filter(issues__volume=volume)
        .annotate(
            issues_count=Count("issues", filter=Q(issues__volume=volume)),
            desc=Concat(Value("Appeared in "), F("issues_count"), Value(" issue(s)"), output_field=TextField()),
...
def get_objects_queryset(volume):
    return (
        Object.objects.filter(comicvine_status="MATCHED")
        .filter(issues__volume=volume)
        .annotate(
            issues_count=Count("issues", filter=Q(issues__volume=volume)),
            desc=Concat(Value("Appeared in "), F("issues_count"), Value(" issue(s)"), output_field=TextField()),
...
def get_authors_queryset(volume):
    return (
        Person.objects.filter(comicvine_status="MATCHED")
        .filter(issues__volume=volume)
        .annotate(
            issues_count=Count("issues", filter=Q(issues__volume=volume)),
            desc=Concat(Value("Appeared in "), F("issues_count"), Value(" issue(s)"), output_field=TextField()),
...
def get_story_arcs_queryset(volume):
    return (
        StoryArc.objects.filter(comicvine_status="MATCHED")
        .filter(issues__volume=volume)
        .distinct()
        .order_by("name", "id")
    )
...
def get_teams_queryset(volume):
    return (
        Team.objects.filter(comicvine_status="MATCHED")
        .filter(issues__volume=volume)
        .annotate(
            issues_count=Count("issues", filter=Q(issues__volume=volume)),
            desc=Concat(Value("Appeared in "), F("issues_count"), Value(" issue(s)"), output_field=TextField()),
...
def get_disbanded_queryset(volume):
    return (
        Team.objects.filter(comicvine_status="MATCHED")
        .filter(disbanded_in_issues__volume=volume)
        .distinct()
        .order_by("name", "id")
    )
...
def get_first_appearance_queryset(volume):
    def get_subquery(model, url_template_name, heading):
        return (
            model.objects.filter(comicvine_status="MATCHED")
            .filter(first_issue__volume=volume)
            .values(
                "name",
                "slug",
